...
async function translateHTMLToPNG(dados) {
  ...
  const codigoDeBarras = (dom.window.document.getElementsByTagName('img')[4].src); // Código de barras
  const codigoQR = (dom.window.document.getElementsByTagName('img')[5].src); // Código de barras

  const arrDataVencimento = vencimentoPostagem.split(" ");
  const arrRemententeSuperior = remententeSuperior.split(" ");

  const postagem = dados.postagem;
  ...
  doc.font('Helvetica-Bold')
    .fontSize(9)
    .fillColor('black')
    .text(`Pedido: ${postagem.codigoInterno}`, 100, 143, {
      width: "120px"
    })

  if(postagem.postagemReversa == 1) {
    doc.rect(270, 141.3, 8, 8)
      .fill();
    doc.font('Helvetica-Bold')
      .fontSize(8)
      .fillColor('white')
  ...
  doc.font('Helvetica')
    .fontSize(8)
    .fillColor('black')
    .text(postagem.destinatario, 120, spaceBetweenAddressInfo.name, {
      width: "120px"
    })
  ...
  doc.font('Helvetica')
    .fontSize(8)
    .fillColor('black')
    .text(postagem.cep, 120, spaceBetweenAddressInfo.zip, {
      width: "120px"
    })
  ...
  doc.font('Helvetica')
    .fontSize(8)
    .fillColor('black')
    .text(postagem.endereco, 120, spaceBetweenAddressInfo.address, {
      width: "120px"
    })
  ...
  doc.font('Helvetica')
    .fontSize(8)
    .fillColor('black')
    .text(postagem.complemento, 120, spaceBetweenAddressInfo.complement, {
      width: "120px"
    })
  ...
  doc.font('Helvetica')
    .fontSize(8)
    .fillColor('black')
    .text(postagem.bairro, 120, spaceBetweenAddressInfo.district, {
      width: "120px"
    })
  ...
  doc.font('Helvetica')
    .fontSize(8)
    .fillColor('black')
    .text(postagem.cidade, 120, spaceBetweenAddressInfo.city, {
      width: "120px"
    })
  doc.font('Helvetica')
    .fontSize(8)
    .fillColor('black')
    .text(postagem.uf, 120, spaceBetweenAddressInfo.fu, {
      width: "120px"
    })
  ...
  doc.font('Helvetica')
    .fontSize(8)
    .fillColor('black')
    .text(postagem.referência, 120, spaceBetweenAddressInfo.reference, {
      width: "120px"
    })
  ...
  doc.font('Helvetica')
    .fontSize(7)
    .fillColor('black')
    .text(postagem.remetente, 14, 404, {
      width: "120px"
    })
  ...
  doc.pipe(fs.createWriteStream(path.join(`temp/${dados.objeto}.pdf`)));
...
async function convertToBase64({ objeto }) {
  ...
  fs.unlink(path.join(`temp/${objeto}.pdf`), () => {});
